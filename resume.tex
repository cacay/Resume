%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Deedy CV/Resume
% XeLaTeX Template
% Version 1.0 (5/5/2014)
%
% This template has been downloaded from:
% http://www.LaTeXTemplates.com
%
% Original author:
% Debarghya Das (http://www.debarghyadas.com)
% With extensive modifications by:
% Vel (vel@latextemplates.com)
%
% License:
% CC BY-NC-SA 3.0 (http://creativecommons.org/licenses/by-nc-sa/3.0/)
%
% Important notes:
% This template needs to be compiled with XeLaTeX.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[letterpaper]{deedy-resume} % Use US Letter paper, change to a4paper for A4 
\usepackage[super]{nth}
\usepackage{setspace}
\renewcommand{\baselinestretch}{1.1} 

\begin{document}

%----------------------------------------------------------------------------------------
%	TITLE SECTION
%----------------------------------------------------------------------------------------

% \lastupdated % Print the Last Updated text at the top right

\namesection{Josh}{Acay}{
%\urlstyle{same}\url{http://www.coskuacay.com} \\
\href{http://www.coskuacay.com}{www.coskuacay.com} \\
\href{mailto:cacay@cmu.edu}{cacay@cmu.edu} | 412-580-7442
}

%----------------------------------------------------------------------------------------
%	LEFT COLUMN
%----------------------------------------------------------------------------------------

\begin{minipage}[t]{0.66\textwidth} % The left column takes up 66% of the text width of the page

%------------------------------------------------
% Education
%------------------------------------------------

\section{Education} 

\runsubsection{Carnegie Mellon University}
\descript{| BS in Computer Science}

\location{Expected May 2016 | Pittsburgh, PA}
\vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightitemize}
	\item GPA: 4.00/4.00
\end{tightitemize}

\sectionspace % Some whitespace after the section


%------------------------------------------------
% Experience
%------------------------------------------------

\section{Experience}

\runsubsection{Union Pacific Railroad}
\descript{| Software Engineering Intern}

\location{June 2014 \textendash{} July 2014 | Omaha, NE}
% \vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightitemize}
\item Setup JIRA project management system for Streamline IT department
\item Implemented responsive dashboards using AngularJS, NodeJS, and NVD3
\item Integrated data from Postgres database and JIRA RESTful API service 
\item Application now being used by IT department and operations
\end{tightitemize}

\sectionspace % Some whitespace after the section

%------------------------------------------------

\runsubsection{Carnegie Mellon University}
\descript{| Teaching Assistant}

\location{Summer 2013 \textendash{} Present | Pittsburgh, PA}
\begin{tightitemize}
\item Lead recitations of 20\textendash{}30 students and prepared course material
\item 15-314: Programming Language Semantics (Spring 2015)
\item 15-210: Parallel and Sequential Data Structures and Algorithms (Fall 2014)
\item 15-150: Functional Programming (Fall 2013 \& Spring 2014)
\item 15-122: Principles of Imperative Computation (Summer 2013)
\end{tightitemize}

\sectionspace % Some whitespace after the section

%------------------------------------------------

\runsubsection{Mobile Robot Competition (Mobot)}
\descript{| Lead Programmer}

\location{Fall 2012 \textendash{} Spring 2014 | Pittsburgh, PA}
\begin{tightitemize}
\item Placed \nth{1} in annual line-tracking competition to receive \$1000 (twice)
\item Built and programmed a mobile robot using Arduino C++
\item Designed a camera-based line detection algorithm in a team of four
\end{tightitemize}

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Experience
%------------------------------------------------

\section{Projects}

\runsubsection{C0 Compiler}
\descript{| Compiler Design}

\location{Fall 2014 | CMU}
\begin{tightitemize}
\item Implemented an optimizing compiler for a subset of C in Haskell
\item Ranked \nth{2} best in code optimization among 40 teams (better than GCC)
\item Extended the language and the compiler with first-class functions
\end{tightitemize}

\sectionspace % Some whitespace after the section

%------------------------------------------------

\runsubsection{Peg Solitaire AI}
\descript{| Principles of Imperative Computation}

\location{Fall 2012 | CMU}
\begin{tightitemize}
\item Implemented a highly optimized AI in C to solve peg solitaire game
\item Ranked \nth{1} in running time among 170 other students
\item Beat the next fastest implementation by a factor of 2 on all boards
\end{tightitemize}

\sectionspace % Some whitespace after the section

%------------------------------------------------

\runsubsection{Senior Thesis}
\descript{| Undegraduate Research}

\location{Spring 2015 \textendash{} Present | Pittsburgh, PA}
\begin{tightitemize}
\item Researching refinement types for concurrent programming languages
\end{tightitemize}

%----------------------------------------------------------------------------------------

\end{minipage} % The end of the left column
\hfill
%
%----------------------------------------------------------------------------------------
%	RIGHT COLUMN
%----------------------------------------------------------------------------------------
%
\begin{minipage}[t]{0.33\textwidth} % The right column takes up 33% of the text width of the page

%------------------------------------------------
% Links
%------------------------------------------------

\section{Links} 

Github:// \href{https://github.com/cacay}{\bf cacay} \\
LinkedIn:// \href{https://www.linkedin.com/in/cacay}{\bf cacay}

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Skills
%------------------------------------------------

\section{Skills}

\subsection{Programming}

C/C++ \textbullet{} Python \textbullet{} Haskell \textbullet{} Standard ML \\
Assembly \textbullet{} \LaTeX\

\sectionspace % Some whitespace after the section

%------------------------------------------------

\subsection{Web Design}

JavaScript \textbullet{} AngularJS \textbullet{} HTML5 \textbullet{} CSS3

\sectionspace % Some whitespace after the section

%------------------------------------------------

\subsection{Platforms}

Mac OS \textbullet{} Linux \textbullet{} Windows \textbullet{} Arduino

\sectionspace % Some whitespace after the section

%------------------------------------------------

\subsection{Languages}

Turkish \textbullet{} English \textbullet{} Japanese (elementary)

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Coursework
%------------------------------------------------

\section{Coursework}

\begingroup
\renewcommand*{\arraystretch}{1.05}
\begin{tabular}{@{}l@{}}
15-411: Compiler Design \\
15-418: Parallel Computer Programming* \\
15-451: Algorithm Design \& Analysis \\
15-312: Foundations of Prog. Languages* \\
15-812: Prog. Language Semantics \\
15-317: Constructive Logic \\
15-210: Data Structures and Algorithms \\
15-381: Artificial Intelligence \\
15-213: Intro to Computer Systems \\
15-251: Great Theoretical Ideas in CS
\end{tabular}
\endgroup

%\begin{flushright}
\null\hfill {\footnotesize \textit{*Current}}
%\end{flushright}

\sectionspace % Some whitespace after the section

\section{Honors}

\descript{Dean's List}
\location{All Semesters | Carnegie Mellon Univ.}
\sectionspace % Some whitespace after the section

\descript{National English Honor Society}
\location{2012 | Uskudar American Academy}
\sectionspace % Some whitespace after the section

\descript{Excellence in Mathematics}
\location{2010, 2011, 2012 | UAA}
\sectionspace % Some whitespace after the section

\descript{Fermat School Champion}
\location{2011 | Uskudar American Academy}
\sectionspace % Some whitespace after the section

\descript{Cayley School Champion}
\location{2010 | Uskudar American Academy}

%----------------------------------------------------------------------------------------

\end{minipage} % The end of the right column

\end{document}